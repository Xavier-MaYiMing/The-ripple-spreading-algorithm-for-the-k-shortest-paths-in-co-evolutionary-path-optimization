...
# @File    : RSA4kSPPCEPO.py
# @Statement : The ripple-spreading algorithm for the k shortest paths in co-evolutionary path optimization
...
def draw_pic(x, y, network, orad, ospeed, t, path, v, ind):
    """
    ...
    :param ind: the index of the shortest path
    """
    path_cost = [0]
    ...
    for time in range(t + 1):
        ...
        plt.xticks([])
        plt.yticks([])
        name = 'index = ' + str(ind) + 'time = ' + str(time) + '.png'
...
def main(network, source, destination, k, x, y, orad, ospeed):
    """
    The main function of the RSA4kSPPCEPO
    :param network: {node1: {node2: length, node3: length, ...}, ...}
    ...
    while len(omega[destination]) < k:
        ...
        for node in incoming_ripples.keys():
            if node in active_node:
                new_ripple = sorted(incoming_ripples[node], key=lambda x: x['radius'],
                                    reverse=True)  # the ripples are ranked by their radii
            else:
                new_ripple = sorted(incoming_ripples[node],
                                    key=lambda x: x['length'])  # the ripples are ranked by their lengths
            if len(omega[node]) + len(new_ripple) > k:
                new_ripple = new_ripple[: k - len(omega[node])]
            for ripple in new_ripple:
                epicenter_set.append(node)
                radius_set.append(ripple['radius'])
                length_set.append(ripple['length'])
                path_set.append(ripple['path'])
                state_set.append(ripple['state'])
    ...
    #     draw_pic(x, y, network, orad, ospeed, t, result[i]['path'], v, i)
    return result
